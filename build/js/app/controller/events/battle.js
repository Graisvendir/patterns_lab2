...
function canAttackerClaimTarget(_attacker, _target) {
    if (_attacker.complexSpeed() > _target.complexSpeed())
        return true;
    return false;
}
function attackerKillTarget(_attacker, _target) {
    console.log('    ' + _attacker.getName + ' win ');
    _attacker.iFeeding();
    _target.iDead();
    _attacker.takePropertyFrom(_target);
}
function targetKillAttacker(_attacker, _target) {
 ...
 */
function battle(_attacker, _target) {
 ...
 */
function attack(_attacker, _target) {
